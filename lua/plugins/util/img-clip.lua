...
return {
  {
    ...
    opts = {
      ...
      default = {
        ...
        -- dir_path = vim.g.neovim_mode == 'skitty' and 'img' or function() return vim.fn.expand '%:t:r' .. '-img' end,
        --
        ...
        -- process_cmd = "magick - -quality 75 png:-", ---@type string

        -- extension = "jpg", ---@type string
        -- process_cmd = "magick - -quality 75 jpg:-", ---@type string

        -- -- Here are other conversion options to play around
        -- -- Notice that with this other option you resize all the images
        -- process_cmd = "magick - -quality 75 -resize 50% png:-", ---@type string
        ...
        -- process_cmd = "magick - -depth 24 -define png:compression-filter=5 -define png:compression-level=9 -define png:compression-strategy=1 png:-",
        --
        -- -- These are for jpegs
        -- process_cmd = "magick - -sampling-factor 4:2:0 -strip -interlace JPEG -colorspace RGB -quality 75 jpg:-",
        -- process_cmd = "magick - -strip -interlace Plane -gaussian-blur 0.05 -quality 75 jpg:-",
        --
      },
      ...
      filetypes = {
        typst = {
          ...
          dir_path = function()
            local cwd = vim.loop.cwd() or vim.fn.getcwd()
            local net_book = vim.fn.expand '~/github/net-book'
            if vim.startswith(cwd, net_book) then return 'assets/img/imgs' end
            return vim.fn.expand '%:t:r' .. '-img'
          end,
