-- Compatibility layer for deprecated util APIs.
-- Redirects old Util helpers to newer implementations (mainly Snacks).

---@class util.deprecated
---
local M = {}
...
M.moved = {
  lsp = {
    ...
    on_supports_method = {
      ...
      fn = function(method, cb)
        return Snacks.util.lsp.on({ method = method }, function(buffer, client) cb(client, buffer) end)
      end,
    },
    on_attach = {
      'Snacks.util.lsp.on',
      fn = function(cb, name)
        return Snacks.util.lsp.on({ name = name }, function(buffer, client) cb(client, buffer) end)
      end,
    },
  },
  terminal = {
  ...
  ui = {
    ...
    foldexpr = { 'Util.treesitter.foldexpr', stacktrace = false },
    fg = {
      '{ fg = Snacks.util.color(...) }',
      fn = function(...) return { fg = Snacks.util.color(...) } end,
    },
  },
}
...
function M.decorate(name, mod)
  if not M.moved[name] then return mod end
  return setmetatable(mod, {
    ...
    __index = function(_, k)
      if M.moved[name][k] then
        ...
        if M.moved[name][k].fn then return M.moved[name][k].fn end
        local ret = vim.tbl_get(_G, unpack(vim.split(to, '.', { plain = true })))
        return ret
      end
      return nil
...
function M.ui() return M.decorate('ui', {}) end
