...
M.formatters = {} ---@type UtilFormatter[]

---@param formatter UtilFormatter
...
---@param buf? number
---@return (UtilFormatter|{active:boolean,resolved:string[]})[]
...
function M.health()
  ...
  local has_none_ls = Util.has("none-ls.nvim")
  if has_plugin and not has_extra then
    Util.warn({
      "`conform.nvim` and `nvim-lint` are now the default formatters and linters in Util.",
      "",
      "You can use those plugins together with `none-ls.nvim`,",
      "but you need to install and enable `none-ls.nvim`,",
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd("BufWritePre", {
    group = vim.api.nvim_create_augroup("util_format", {}),
